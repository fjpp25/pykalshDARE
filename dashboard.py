...
Supplementary: btc_prices.csv (σ calibration tab)
               positions_full.csv (Positions tab — run position_tracker.py)
...
from PyQt6.QtWidgets import (
    ...
    QFrame, QSplitter, QPushButton, QTabWidget, QPlainTextEdit,
)
...
# ========================= CONFIG =========================
POSITIONS_FILE = "positions_full.csv"   # written by position_tracker.py
PRICES_FILE    = "btc_prices.csv"       # written by main.py
REFRESH_MS     = 15_000
LOCAL_TZ_OFFSET = 1   # UTC+1 (WEST) — display only, never used for session logic
...
def fetch_kalshi_positions() -> list[dict]:
    """
    Build position history from Kalshi's /portfolio/fills and
    /portfolio/settlements endpoints.  Returns a list of dicts whose
    keys match the schema used by RawLogTab and OverviewTab:

        ticker, ticker_display, first_trade_time, utc_hour, session,
        side, count, avg_price, result, outcome, pnl, exit_reason,
        market_status
    """
    # ── 1. Fetch all fills ──
    all_fills: list[dict] = []
    ...
    # ── 4. Fetch market status for unsettled tickers ──
    market_cache: dict[str, dict] = {}
    for ticker in (t for t in by_ticker if t not in all_settlements):
        try:
    ...
    for ticker, fills in by_ticker.items():
        ...
        try:
            if settlement and result in ("yes", "no"):
                ...
                cost_paid = _fp(settlement.get(
                    "yes_total_cost_dollars" if side == "yes"
                    else "no_total_cost_dollars"
                ))
        ...
        # Session uses UTC consistently — same as main.py's CSV logs
        utc_hour, sess, ts_fmt = 0, "unknown", ""
        if first_time:
            try:
                dt       = datetime.fromisoformat(first_time.replace("Z", "+00:00"))
                utc_hour = dt.hour
                sess     = get_session_name(utc_hour)   # UTC, no offset
...
def load_positions_full() -> list[dict]:
    """
    Load positions_full.csv written by position_tracker.py.
...
def load_csv(path: str) -> list[dict]:
    """Load an arbitrary CSV into a list of dicts."""
...
class StatCard(QFrame):
    def __init__(self, title: str, value: str = "—", subtitle: str = "",
        ...
        self._title_lbl.setStyleSheet(
            ...
            f"letter-spacing: 2px; font-family: {SANS};"
        )
        self._value_lbl = QLabel(value)
        self._value_lbl.setStyleSheet(
            f"color: {color}; font-size: 26px; "
            f"font-family: {MONO}; font-weight: bold;"
        )
        self._sub_lbl = QLabel(subtitle)
        self._sub_lbl.setStyleSheet(
            f"color: {C['text_dim']}; font-size: 11px; font-family: {MONO};"
        )
...
class SectionHeader(QLabel):
    def __init__(self, text: str, parent=None):
        ...
        self.setStyleSheet(
            ...
            f"font-family: {SANS}; font-weight: bold; padding: 8px 0 4px 0; "
...
def make_table(headers: list[str], row_height: int = 32) -> QTableWidget:
    ...
    t.setStyleSheet(f"QTableWidget::item:alternate {{ background: {C['bg3']}; }}")
    return t
...
class OverviewTab(QWidget):
    def __init__(self, parent=None):
        ...
        for card in (self.card_trades, self.card_winrate, self.card_pnl,
        ...
        self.side_table = make_table(
            ["Side", "Trades", "Wins", "Losses", "Win Rate", "Kelly Preview"]
        )
        ...
        self.recent_table = make_table([
            "Time (local)", "Ticker", "Session", "Side",
    ...
    def refresh(self, positions: list[dict], stats: dict):
        ...
        wr_color     = C["green"] if wr >= 0.6 else C["yellow"] if wr >= 0.45 else C["red"]
        ...
        recent = [p for p in positions if p.get("outcome") != "unknown"][-20:][::-1]
        ...
        for i, p in enumerate(recent):
            ...
            o_color    = (C["green"] if outcome == "win" else C["red"] if outcome == "loss"
                          else C["blue"] if outcome == "open" else C["yellow"])
            side_color = (C["blue"] if side == "yes" else C["purple"] if side == "no"
                          else C["text_dim"])
...
class PositionsTab(QWidget):
    """
    Full reconciled position history.
    Data comes from positions_full.csv (written by position_tracker.py).
    Shows a prompt to run position_tracker.py if the file is absent.
    """
    ...
    def __init__(self, parent=None):
        ...
        self.pos_summary = QLabel(
            "Run position_tracker.py to populate this tab."
        )
        ...
        layout.addWidget(SectionHeader("All Positions (from positions_full.csv)"))
    ...
    def refresh(self, positions: list[dict]):
        """positions is loaded from positions_full.csv via load_positions_full()."""
        if not positions:
            age = ""
            if os.path.exists(POSITIONS_FILE):
                secs = (datetime.now() - datetime.fromtimestamp(
                    os.path.getmtime(POSITIONS_FILE)
                )).total_seconds()
                age = f" (last updated {int(secs)}s ago)"
            self.pos_summary.setText(
                f"positions_full.csv not found — run position_tracker.py{age}"
            )
        ...
        self.pos_summary.setText(
            f"{len(positions)} total  |  {len(settled)} settled  |  "
            f"{wins}W / {len(settled)-wins}L  |  "
            f"PnL: ${total_pnl:+.2f}  |  {mismatches} mismatches"
        )
        ...
        for i, p in enumerate(sorted_pos):
            ...
            r_color    = (C["green"] if result == "yes" else C["red"] if result == "no"
                          else C["text_dim"])
            side_color = C["blue"] if side == "yes" else C["purple"] if side == "no" else C["text_dim"]

            # Display timestamp in local time
            ts_raw = p.get("first_trade_time", "")
            try:
                dt     = datetime.fromisoformat(ts_raw.replace("Z", "+00:00"))
                ts_fmt = dt.replace(hour=(dt.hour + LOCAL_TZ_OFFSET) % 24).strftime("%m-%d %H:%M")
            except Exception:
                ts_fmt = ts_raw[:16]

            self.table.setItem(i,  0, make_item(ts_fmt, C["text_dim"]))
            self.table.setItem(i,  1, make_item(p.get("ticker", "").replace("KXBTC15M-", "")))
...
class SigmaTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.sigma_table = make_table(
            ["Session", "σ/min", "Fat-tail k", "α (exponent)", "Status"]
        )
    ...
    def refresh(self, prices: list[dict]):
        ...
        for i, sess in enumerate(sessions):
            ...
            d_s, _, _ = defaults[sess]
            calibrated = abs(s - d_s) > 1e-7
...
class PnLTab(QWidget):
    ...
    def refresh(self, stats: dict):
        ...
        for i, p in enumerate(reversed(series)):
            ...
            self.table.setItem(i, 6, make_item(f"{p['drawdown']:+.4f}", dd_c2))
            self.table.setItem(i, 7, make_item(p["outcome"].upper(), o_c, bold=True))


class _PnLCanvas(QWidget):
    """Three vertically stacked mini-charts: cumulative PnL, per-trade bars, drawdown."""
    ...
    def paintEvent(self, event):
        ...
        def x_pos(i: int) -> float:
            return PAD_L + (chart_w / 2 if n <= 1 else i / (n - 1) * chart_w)
        ...
        def gridline(y: float, val: float):
        ...
        for i in range(n - 1):
            poly = QPolygonF([
                QPointF(x_pos(i),     zero_y),
                QPointF(x_pos(i),     cum_y(cum_vals[i])),
                QPointF(x_pos(i + 1), cum_y(cum_vals[i + 1])),
                QPointF(x_pos(i + 1), zero_y),
            ])
            fill = QColor(C["green_dim"] if cum_vals[i] >= 0 else C["red_dim"])
        ...
        for i, p in enumerate(series):
            ...
            if v >= 0:
                painter.drawRect(_QRectF(x, bar_zero - h, bar_w, h))
            else:
                painter.drawRect(_QRectF(x, bar_zero, bar_w, h))
...
class RawLogTab(QWidget):
    """Live position history pulled directly from the Kalshi API."""
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(SectionHeader("Full Position History (Kalshi API)"))
        self.table = make_table([
            "Time (local)", "Ticker", "Session", "Side",
    ...
    def refresh(self, positions: list[dict]):
        ...
        for i, p in enumerate(rows):
            ...
            side_color = C["blue"] if side == "yes" else C["purple"] if side == "no" else C["text_dim"]
            r_color    = C["green"] if result == "yes" else C["red"] if result == "no" else C["text_dim"]
...
class LogConsoleTab(QWidget):
    ...
    def append(self, line: str):
        ...
        self.console.appendHtml(f'<span style="color:{colour};">{escaped}</span>')
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        header.setStyleSheet(
            f"background: {C['bg3']}; border-bottom: 1px solid {C['border2']};"
        )
        ...
        title.setStyleSheet(
            f"color: {C['accent']}; font-size: 15px; "
            f"font-family: {MONO}; font-weight: bold; letter-spacing: 4px;"
        )
        ...
        self.start_btn.setStyleSheet(
            ...
            f"border: 1px solid {C['green']}; padding: 6px 16px; "
            f"font-family: {SANS}; font-size: 11px; letter-spacing: 1px; }}"
            f"QPushButton:hover {{ background: {C['green']}; color: {C['bg']}; }}"
            f"QPushButton:disabled {{ background: {C['bg3']}; "
            f"color: {C['text_dim']}; border-color: {C['border']}; }}"
        )
        ...
        self.stop_btn.setStyleSheet(
            f"QPushButton {{ background: {C['red_dim']}; color: {C['red']}; "
            f"border: 1px solid {C['red']}; padding: 6px 16px; "
            f"font-family: {SANS}; font-size: 11px; letter-spacing: 1px; }}"
            f"QPushButton:hover {{ background: {C['red']}; color: {C['bg']}; }}"
            f"QPushButton:disabled {{ background: {C['bg3']}; "
            f"color: {C['text_dim']}; border-color: {C['border']}; }}"
        )
    ...
    def _start_bot(self):
        self.log_tab.append(
            f"[DASHBOARD] Starting bot — {self._now_str()}"
        )
    ...
    def _stop_bot(self):
        self.log_tab.append(
            f"[DASHBOARD] Stopping bot — {self._now_str()}"
        )
        self._bot.stop()
    ...
    def _on_bot_stopped(self):
        ...
        self.stop_btn.setEnabled(False)
        self.log_tab.append(f"[DASHBOARD] Bot process ended — {self._now_str()}")
    ...
    def _now_str(self) -> str:
        ...
        return now.strftime(f"%Y-%m-%d {local_hour:02d}:%M:%S local")
    ...
    def load_data(self):
        ...
        try:
            api_positions = fetch_kalshi_positions()
            api_ok        = True
        except Exception as e:
            api_positions = []
            api_ok        = False
        ...
        # Reconciled positions from position_tracker.py CSV
        reconciled_positions = load_positions_full()

        prices = load_csv(PRICES_FILE)
        stats  = calc_stats(api_positions)

        self.overview_tab.refresh(api_positions, stats)
        self.positions_tab.refresh(reconciled_positions)   # ← correct source
        self.pnl_tab.refresh(stats)
        self.sigma_tab.refresh(prices)
        self.rawlog_tab.refresh(api_positions)

        now     = datetime.now(timezone.utc)
        now_str = self._now_str()
        suffix  = "" if api_ok else " ⚠ API ERROR"
        self.last_refresh_lbl.setText(f"Last refresh: {now_str}{suffix}")

        settled  = sum(1 for p in api_positions if p["outcome"] in ("win", "loss"))
        opens    = sum(1 for p in api_positions if p["outcome"] == "open")
        btc_rows = len(prices)
        btc_age  = ""
        ...
        pos_note = (
            f"positions_full: {len(reconciled_positions)} rows"
            if reconciled_positions else
            "positions_full: not found"
        )
        self.file_status_lbl.setText(
            f"Kalshi API: {len(api_positions)} positions  |  "
            f"{settled} settled  |  {opens} open  |  "
            f"btc_prices: {btc_rows} rows{btc_age}  |  {pos_note}"
        )
